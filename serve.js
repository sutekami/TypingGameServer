...
io.on('connection', socket => {
  ...
  socket.on('changeResult', data => {
    for (let i of room_info) {
      if (i.roomId === data.roomId) {
        (i.user1.token === data.token) ? io.to(i.user2.token).emit('changeResult')
          : io.to(i.user1.token).emit('changeResult');
      }
    }
  })
  ...
  socket.on('disconnect', () => {
    for (let i = 0; i < room_info.length; i++) {
      if (room_info[i].user1.token === socket.id) {
        room_info.splice(i, 1);
        break;
      } else if (room_info[i].user2.token === socket.id) {
        room_info.splice(i, 1);
        break;
      }
    }
